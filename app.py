...
def credit(an,pn):
    if request.method=='POST':
        ...
        details=data[an]   #if accountno is 343434 then details {'pinno':111,'balance':2020}
        ...
        if data_list[0]['account_no']==an:  #validating accno
            orginal_amount=data_list[0]['balance']  # fetching original amount
            data[an]['balance']=orginal_amount+amount  #incrementing amount
            print(data_list)
...
def withdraw(an,pn):
    ...
    return render_template('withdraw.html',an=an,pn=pn)
